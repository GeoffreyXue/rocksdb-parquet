...
extern std::string MakeParquetFileName(const std::string& name, uint64_t number);
...
// Return the name of the pqruet with the specified number
// in the db named by "dbname".  The result will be prefixed with
// "dbname".
extern std::string ParquetFileName(const std::vector<DbPath>& db_paths,
                                 uint64_t number, uint32_t path_id);
